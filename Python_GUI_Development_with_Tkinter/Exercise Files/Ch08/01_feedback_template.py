...
class Feedback:
    ...
    def __init__(self, master):
        ...
        self.logo = PhotoImage(
            file= '/Users/sedsaid/Documents/Lessons/python_learning_path/Python_GUI_Development_with_Tkinter/Exercise Files/Ch08/tour_logo.gif'
        )
        ttk.Label(self.frame_header, image = self.logo).grid(row = 0,
            column = 0, rowspan = 2)
        ttk.Label(self.frame_header, text = "Thanks for Exploring!" ).grid(
            row = 0, column = 1)
        ttk.Label(self.frame_header, wraplength = 300,
                text = "We're glad you chose Explore California for your           recent adventrue."
                "Please tell us what you thought about the 'Desert to Sea' tour. " ).grid(row = 1, column = 1)

        self.frame_content = ttk.Frame(master)
        self.frame_content.pack()

        ttk.Label(self.frame_content, text = 'Name:').grid(row = 0,
            column = 0, padx = 5, sticky = 'sw')
        ttk.Label(
            self.frame_content, text='Email:').grid(
                row=0, column=1, padx=5, sticky='sw')
        ttk.Label(
            self.frame_content, text='Comments:').grid(
        ...
        ttk.Button(
            self.frame_content, text='Submit', command = self.submit).grid(
                row=4, column=0, padx=5, sticky = 'e')
        ttk.Button(
            self.frame_content, text='Clear', command = self.clear).grid(
